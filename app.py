...
def analyse(image):
    ...
    sos_text = "SHARE OF SHELF\n" + "="*35 + "\n"
    for name, count in det_counts.items():
        pct = count / total_count * 100 if total_count > 0 else 0
        bar = "█" * int(pct / 5)
        sos_text += f"{name:<15}: {count} facings  {pct:.1f}%  {bar}\n"
    sos_text += "\n" + "="*35 + "\n"
    sos_text += f"Coca-Cola SOS: {sos:.1f}%\n"
    ...
    comp_text = "PLANOGRAM COMPLIANCE\n" + "="*35 + "\n"
    comp_text += f"Matching : {matching_cells}/{total_cells}\n"
    comp_text += f"Score    : {compliance:.1f}%\n"
...
with gr.Blocks(title="ShelfSight", theme=gr.themes.Base()) as demo:
    gr.Markdown("""
    # ShelfSight — AI Shelf Intelligence
    **CSC-233 AI Lab | BNU Spring 2026 | YOLOv8n+Augmentation | mAP50: 95.06%**
    ...
    with gr.Row():
        with gr.Column():
            ...
            btn = gr.Button("Analyse Shelf", variant="primary", size="lg")
        with gr.Column():
            image_output = gr.Image(label="Detected products")
    with gr.Row():
        sos_out  = gr.Textbox(label="Share of Shelf",        lines=10)
        comp_out = gr.Textbox(label="Planogram Compliance",  lines=10)
        reco_out = gr.Textbox(label="Recommendations",       lines=10)
    btn.click(fn=analyse, inputs=image_input, outputs=[image_output, sos_out, comp_out, reco_out])
